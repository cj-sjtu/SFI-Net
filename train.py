...
class Supervision_Train(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Debug: print mask value ranges for first batch
        if batch_idx == 0:  # Only print for first batch to avoid spam
            print(f"Debug - training mask min: {mask.min()}, max: {mask.max()}, unique values: {torch.unique(mask)}")
            print(f"Debug - training pre_mask min: {pre_mask.min()}, max: {pre_mask.max()}, unique values: {torch.unique(pre_mask)}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Debug: print mask value ranges
        if batch_idx == 0:  # Only print for first batch to avoid spam
            print(f"Debug - mask min: {mask.min()}, max: {mask.max()}, unique values: {torch.unique(mask)}")
            print(f"Debug - pre_mask min: {pre_mask.min()}, max: {pre_mask.max()}, unique values: {torch.unique(pre_mask)}")
