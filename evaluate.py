    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            for i in range(raw_predictions.shape[0]):
                ...
                masks_true = cv2.morphologyEx(masks_true, cv2.MORPH_CLOSE, kernel)
                
                save_dir = os.path.join(args.output_path,str(image_ids[0])+"_res"+".jpg")
                mask_dir =os.path.join(args.output_path,str(image_ids[0])+"_label"+".jpg")
                cv2.imwrite(save_dir,mask)
                cv2.imwrite(mask_dir,masks_true)
